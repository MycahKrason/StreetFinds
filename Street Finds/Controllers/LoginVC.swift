...
class LoginVC: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        //Move display when keyboard is shown
        view.bindToKeyboard()
        ...
        let selectedSegmentAttribute = [NSAttributedString.Key.foregroundColor: UIColor.black]
        segmentedControlBtn.setTitleTextAttributes(selectedSegmentAttribute, for: .selected)
        let normalSegmentAttribute = [NSAttributedString.Key.foregroundColor: UIColor.gray]
        segmentedControlBtn.setTitleTextAttributes(normalSegmentAttribute, for: .normal)
                        ...
                        Auth.auth().createUser(withEmail: email, password: password) { (user, error) in
                            ...
                            if error != nil{
                                ...
                                if let user = user{
                                    ...
                                    //Create users
                                    //user is auto set to isActive true - make sure you change this to false until you build the option
                                    let userData = ["Email": email, "UserID": user.user.uid, "isActive": false, "VendorName" : "Vendor Name", "VendorDescription" : "Describe what you sell or do."] as [String: Any]
    ...
    func navigateCustomerOrVendor(){
        //navigate to the appropriate page depending on the segmentedControl
        if self.segmentedControlBtn.selectedSegmentIndex == 0{
            //Go to the Map
            DataService.instance.REF_USERS.child((Auth.auth().currentUser?.uid)!).updateChildValues(["isActive": false])
            performSegue(withIdentifier: "segueMapVC", sender: nil)
            
        }else{
            //Go to the Vendor Profile
            performSegue(withIdentifier: "segueVendorProfileVC", sender: nil)
        }
    }
